"use client";

import { MainfudContext } from "@/context/MainfudContext";
import { MasterContext } from "@/context/MasterContext";
import { AnimatePresence, motion } from "framer-motion";
import { ChevronDown } from "lucide-react";
import Image from "next/image";
import React, { useContext, useEffect, useState } from "react";
import { AiOutlineProduct } from "react-icons/ai";
import { HiOutlineCamera, HiOutlineCurrencyDollar } from "react-icons/hi2";
import { IoClose } from "react-icons/io5";
import { PiChatText } from "react-icons/pi";
import { TbFileTextSpark } from "react-icons/tb";
import { toast } from "sonner";
import { Listbox } from "@headlessui/react";

const ModalCrearProducto = () => {
  const {
    openModalProducto,
    setOpenModalProducto,
    producto,
    setProducto,
    formDatosProducto,
    setFormDatosProducto,
    setIngrediente,
  } = useContext(MasterContext);
  const { productos, setProductos, categorias, setIngredientes } =
    useContext(MainfudContext);

  const [file, setFile] = useState(null);
  const [preview, setPreview] = useState(null);
  const [loading, setLoading] = useState(false);
  const [productoInicial, setProductoInicial] = useState(null);

  const [selected, setSelected] = useState(null);

  useEffect(() => {
    if (producto) {
      setFormDatosProducto({
        nombre: producto.nombre,
        precio: producto.precio,
        image: producto.image,
        publicId: producto.publicId,
        categoria: producto.categoria,
        descripcion: producto.descripcion,
        ingredientes: producto.ingredientes || [],
        opcion: "editar", // Cambia a 'editar' si estás editando un producto
        disponible_comer_aqui: producto.disponible_comer_aqui,
        disponible_para_llevar: producto.disponible_para_llevar,
        insumos: producto.insumos || [],
      });
      setProductoInicial(producto);
      setFile(null); // Resetea el archivo para evitar problemas con la previsualización
      setPreview(producto.image); // Usa la imagen actual de la categoría para la previsualización
    } else {
      setFormDatosProducto({
        nombre: "",
        precio: "",
        image: "",
        publicId: "",
        categoria: "",
        descripcion: "",
        ingredientes: [],
        opcion: "crear", // Cambia a 'crear' si estás creando un nuevo producto
        disponible_comer_aqui: true,
        disponible_para_llevar: true,
        insumos: [],
      });
      setFile(null);
      setPreview(null);
    }
  }, [producto, setFormDatosProducto]);

  const handleChange = (e) => {
    const selectedFile = e.target.files[0];
    if (selectedFile) {
      setFile(selectedFile);
      setPreview(URL.createObjectURL(selectedFile));
    }
  };

  const toggleCheckbox = (name) => {
    setFormDatosProducto((prev) => ({
      ...prev,
      [name]: !prev[name],
    }));
  };

  const handleDatosChange = (e) => {
    const { name, value } = e.target;
    setFormDatosProducto((prevDatos) => ({
      ...prevDatos,
      [name]: value,
    }));
  };

  useEffect(() => {
    if (openModalProducto) {
      document.body.style.overflow = "hidden";
    } else {
      document.body.style.overflow = "";
    }
  }, [openModalProducto]);

  const handleUpload = async (e) => {
    e.preventDefault();
    if (!producto) {
      if (!file) {
        toast.error("Por favor, selecciona una imagen.", {
          position: "top-center",
          style: {
            background: "#DBEAFE",
            color: "#1B42A8",
            borderColor: "#1B42A8",
          },
        });
        return;
      }
      if (!formDatosProducto.nombre.trim()) {
        toast.error("Por favor, ingresa una categoria.", {
          position: "top-center",
          style: {
            background: "#DBEAFE",
            color: "#1B42A8",
            borderColor: "#1B42A8",
          },
        });
        return;
      }
    }

    setLoading(true);

    const formData = new FormData();
    formData.append("file", file);
    formData.append("nombre", formDatosProducto.nombre);
    formData.append("precio", formDatosProducto.precio);
    formData.append("categoria", formDatosProducto.categoria);
    formData.append("descripcion", formDatosProducto.descripcion);
    formData.append(
      "ingredientes",
      JSON.stringify(formDatosProducto.ingredientes)
    );
    formData.append("opcion", formDatosProducto.opcion); // 'crear' o 'editar'
    if (formDatosProducto.opcion === "editar") {
      formData.append("productoId", producto._id); // el ID de la categoría que vas a editar
      formData.append(
        "disponible_comer_aqui",
        formDatosProducto.disponible_comer_aqui
      );
      formData.append(
        "disponible_para_llevar",
        formDatosProducto.disponible_para_llevar
      );
      formData.append("insumos", JSON.stringify(formDatosProducto.insumos));
    }

    try {
      const res = await fetch("/api/upload/producto", {
        method: "POST",
        body: formData,
      });

      const data = await res.json();

      if (!res.ok) {
        console.error("Error al subir:", data);
        throw new Error(data.details || data.error || "Error desconocido");
      }
      const nuevoProducto = data.producto;
      setProducto(data.producto);
      setProductos((prevProducto) => {
        const index = prevProducto.findIndex(
          (cat) => cat._id === nuevoProducto._id
        );

        if (index !== -1) {
          // Ya existe → reemplazar
          const actualizadas = [...prevProducto];
          actualizadas[index] = nuevoProducto;
          return actualizadas;
        } else {
          // No existe → agregar
          return [...prevProducto, nuevoProducto];
        }
      });
      toast.success(data.message, {
        duration: 3000,
        position: "top-right",
        style: {
          backgroundColor: "#34d777", // Green color for success
          color: "#000",
          borderColor: "#000", // Darker green border
        },
      });
      setOpenModalProducto(false);
      setFormDatosProducto({
        nombre: "",
        precio: "",
        image: "",
        publicId: "",
        categoria: "",
        descripcion: "",
        ingredientes: [],
        opcion: "crear", // Resetea a 'crear' para futuros productos
        disponible_comer_aqui: true,
        disponible_para_llevar: true,
        insumos: [],
      });
      setFile(null);
      setPreview(null);
    } catch (err) {
      toast.error(`Error al subir la categoría: ${err.message}`, {
        position: "top-center",
        style: {
          background: "#FEE2E2", // Light red background
          color: "#B91C1C", // Dark red text
          borderColor: "#B91C1C", // Dark red border
        },
      });
    } finally {
      setLoading(false);
    }
  };

  const nombreCategoria = (cat) => {
    const categoria = categorias?.find(
      (categoria) => categoria.nombreFormateado === cat
    );
    return categoria ? categoria.nombre : "Categoría no encontrada";
  };

  const nombreCat = nombreCategoria(formDatosProducto.categoria);

  const handleDelete = async (e) => {
    e.preventDefault();
    if (!producto) {
      toast.error("No hay un producto seleccionado", {
        position: "top-center",
        style: {
          background: "#DBEAFE",
          color: "#1B42A8",
          borderColor: "#1B42A8",
        },
      });
      return;
    }
    setLoading(true);

    try {
      const res = await fetch(`/api/productos/${producto._id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          productoId: producto._id, // el ID del producto al que pertenece el ingrediente
        }),
      });
      const data = await res.json();
      if (!res.ok) {
        console.error("Error al eliminar:", data);
        throw new Error(data.details || data.error || "Error desconocido");
      }
      toast.success(data.message, {
        duration: 3000,
        position: "top-right",
        style: {
          backgroundColor: "#34d777", // Green color for success
          color: "#000",
          borderColor: "#000", // Darker green border
        },
      });
      setIngredientes(data.ingredientes);
      setProductos(data.productos);
      setOpenModalProducto(false);
      setFormDatosProducto({
        nombre: "",
        precio: "",
        image: "",
        publicId: "",
        categoria: "",
        descripcion: "",
        ingredientes: [],
        opcion: "crear", // Resetea a 'crear' para futuros productos
        disponible_comer_aqui: true,
        disponible_para_llevar: true,
        insumos: [],
      });
      setFile(null);
      setPreview(null);
      setIngrediente(null); // Resetea el ingrediente seleccionado
    } catch (err) {
      toast.error(`Error al eliminar el producto: ${err.message}`, {
        position: "top-center",
        style: {
          background: "#FEE2E2", // Light red background
          color: "#B91C1C", // Dark red text
          borderColor: "#B91C1C", // Dark red border
        },
      });
    } finally {
      setLoading(false);
    }
  };

  return (
    <AnimatePresence>
      {openModalProducto && (
        <div className="fixed inset-0 z-30 flex items-center bg-black/10 justify-center bg-opacity-90 overflow-auto">
          <motion.div
            className="relative w-full flex flex-col overflow-y-auto mx-auto h-full items-center justify-center"
            initial={{ opacity: 0, scale: 0 }}
            animate={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.3 }}
            exit={{ opacity: 0, scale: 0 }}
            onClick={() => setOpenModalProducto(false)}
          >
            <div
              onClick={(e) => e.stopPropagation()}
              className="w-96 cursor-pointer select-none relative"
            >
              <button
                className="w-10 h-10 rounded-full flex items-center justify-center bg-white absolute -top-12 right-0 z-10"
                onClick={() => {
                  setOpenModalProducto(false);
                }}
              >
                <IoClose className="text-lg text-zinc-800 cursor-pointer" />
              </button>
              <div
                className={`p-4 rounded-2xl shadow-lg bg-white text-zinc-800`}
              >
                <form className="bg-white p-6 text-zinc-800">
                  <h2 className="w-full font-semibold text-2xl text-center mb-6">
                    {producto ? "Editar Producto" : "Agregar Producto"}
                  </h2>
                  <div className="flex flex-col items-center gap-4 mb-10">
                    <div className="w-32 h-32 bg-white rounded-lg border-dashed border-[1px] border-zinc-800 flex items-center justify-center shadow-md relative p-0.5">
                      <Image
                        src={preview || "/master/categorias/categorias.png"}
                        alt="Avatar"
                        width={500}
                        height={500}
                        className="w-full h-full rounded-lg object-cover"
                      />
                      <div className="absolute bottom-0 right-0 bg-rose-500 text-white p-2 rounded-full hover:bg-rose-400 transition-colors cursor-pointer select-none active:scale-95 duration-150 overflow-hidden">
                        {/* Icono visible */}
                        <HiOutlineCamera className="pointer-events-none cursor-pointer" />

                        {/* Input oculto pero funcional */}
                        <input
                          id="cameraInput"
                          type="file"
                          accept="image/*"
                          capture="environment"
                          className="absolute inset-0 opacity-0 cursor-pointer peer"
                          onChange={handleChange}
                        />
                      </div>
                    </div>
                  </div>
                  {/* inputs textp */}
                  <div className="grid gap-6">
                    <div className="grid gap-2 border-[1px] border-border rounded-full px-4 py-3 relative">
                      <span className="text-xs bg-white absolute left-4 -top-2 px-2">
                        Nombre
                      </span>
                      <div className="relative flex items-center gap-4">
                        <TbFileTextSpark className="h-4 w-4 text-muted-foreground" />
                        <input
                          id="nombre"
                          type="text"
                          placeholder="Ejemplo: Empanada de carne"
                          value={formDatosProducto.nombre}
                          onChange={handleDatosChange}
                          required
                          name="nombre"
                          className="w-full bg-transparent text-black placeholder:text-muted-foreground focus:outline-none focus:ring-0 focus:border-purple-600 text-sm"
                        />
                      </div>
                    </div>
                    <div className="grid gap-2 border-[1px] border-border rounded-full px-4 py-3 relative">
                      <span className="text-xs bg-white absolute left-4 -top-2 px-2">
                        Precio
                      </span>
                      <div className="relative flex items-center gap-4">
                        <HiOutlineCurrencyDollar className="h-4 w-4 text-muted-foreground" />
                        <input
                          id="precio"
                          type="text"
                          placeholder="Ejemplo: $ 2500"
                          value={formDatosProducto.precio}
                          onChange={handleDatosChange}
                          required
                          name="precio"
                          className="w-full bg-transparent text-black placeholder:text-muted-foreground focus:outline-none focus:ring-0 focus:border-purple-600 text-sm"
                        />
                      </div>
                    </div>

                    <div className="relative w-full">
                      <Listbox
                        value={selected}
                        onChange={(cat) =>
                          setFormDatosProducto((prev) => ({
                            ...prev,
                            categoria: cat,
                          }))
                        }
                      >
                        {({ open }) => (
                          <div>
                            <Listbox.Button className="w-full border border-border rounded-full px-4 py-3 text-left text-sm bg-white text-gray-700 relative">
                              {formDatosProducto.categoria ? (
                                nombreCat
                              ) : (
                                <span className="flex items-center gap-3">
                                  <AiOutlineProduct className="w-5 h-5 text-gray-500" />{" "}
                                  Seleccione una categoría
                                </span>
                              )}
                              {/* Flecha a la derecha */}
                              <span className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none">
                                <ChevronDown className="w-4 h-4 text-gray-500" />
                              </span>
                            </Listbox.Button>

                            {open && (
                              <Listbox.Options className="absolute z-10 mt-2 w-full bg-white border border-gray-200 rounded-md shadow-lg max-h-60 overflow-y-auto text-sm">
                                {categorias.map((cat, index) => (
                                  <Listbox.Option
                                    key={index}
                                    value={cat.nombreFormateado}
                                    className={({ active, selected }) =>
                                      `cursor-pointer px-4 py-2 ${
                                        active ? "bg-rose-100" : ""
                                      } ${
                                        selected
                                          ? "font-semibold text-blue-600"
                                          : ""
                                      }`
                                    }
                                  >
                                    {cat.nombre}
                                  </Listbox.Option>
                                ))}
                              </Listbox.Options>
                            )}
                          </div>
                        )}
                      </Listbox>
                    </div>

                    {/* Descripcion */}
                    <div className="grid gap-2 border-[1px] border-border rounded-full px-4 py-3 relative">
                      <span className="text-xs bg-white absolute left-4 -top-2 px-2">
                        Descripcion
                      </span>
                      <div className="relative flex items-center gap-4">
                        <PiChatText className="h-4 w-4 text-muted-foreground" />
                        <input
                          id="descripcion"
                          type="text"
                          placeholder="Ejemplo: Deliciosa empanada de carne con especias"
                          value={formDatosProducto.descripcion}
                          onChange={handleDatosChange}
                          required
                          name="descripcion"
                          className="w-full bg-transparent text-black placeholder:text-muted-foreground focus:outline-none focus:ring-0 focus:border-purple-600 text-sm"
                        />
                      </div>
                    </div>
                    <div className="flex flex-col gap-3 mb-6">
                      <label className="flex items-center gap-3">
                        <input
                          type="checkbox"
                          checked={formDatosProducto.disponible_para_llevar}
                          onChange={() =>
                            toggleCheckbox("disponible_para_llevar")
                          }
                          className="appearance-none w-5 h-5 rounded-full border-2 border-zinc-700 checked:bg-green-500 checked:border-green-600 transition-colors"
                        />
                        <span className="text-sm">Disponible para llevar</span>
                      </label>
                      <label className="flex items-center gap-3">
                        <input
                          type="checkbox"
                          checked={formDatosProducto.disponible_comer_aqui}
                          onChange={() =>
                            toggleCheckbox("disponible_comer_aqui")
                          }
                          className="appearance-none w-5 h-5 rounded-full border-2 border-zinc-700 checked:bg-green-500 checked:border-green-600 transition-colors"
                        />
                        <span className="text-sm">
                          Disponible para comer aquí
                        </span>
                      </label>
                    </div>

                    <div className="flex items-center flex-col gap-2 w-full">
                      {producto && (
                        <button
                          type="submit"
                          onClick={handleUpload}
                          className="font-semibold bg-rose-500 rounded-full text-white p-2 hover:bg-rose-400 transition-colors cursor-pointer select-none active:scale-95 duration-150 w-full"
                        >
                          Editar
                        </button>
                      )}
                      {!producto && (
                        <button
                          type="submit"
                          onClick={handleUpload}
                          // disabled={loading || !file}
                          className="font-semibold bg-rose-500 rounded-full text-white p-2 hover:bg-rose-400 transition-colors cursor-pointer select-none active:scale-95 duration-150 w-full"
                        >
                          {loading ? "Subiendo..." : "Crear"}
                        </button>
                      )}
                      {producto && (
                        <button
                          type="submit"
                          onClick={handleDelete}
                          className="font-semibold bg-rose-500 rounded-full text-white p-2 hover:bg-rose-400 transition-colors cursor-pointer select-none active:scale-95 duration-150 w-full"
                        >
                          Eliminar
                        </button>
                      )}
                    </div>
                  </div>
                </form>
              </div>
            </div>
          </motion.div>
        </div>
      )}
    </AnimatePresence>
  );
};

export default ModalCrearProducto;
